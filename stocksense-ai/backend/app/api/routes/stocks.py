...
from app.core.stocks import STOCK_UNIVERSE, get_all_tickers
from app.core.redis_client import cache_get, cache_set
from app.data.moomoo_fetcher import fetch_realtime_quote, fetch_tick_data, fetch_order_book

router = APIRouter()

# In-memory watchlist (swap for DB-backed in production)
_watchlist: list[str] = []
...
async def get_stock(ticker: str):
    # Support both old format (AAPL) and new moomoo format (US.AAPL)
    t = ticker.upper()
    ...
    # Try prefix lookup (e.g., AAPL -> US.AAPL)

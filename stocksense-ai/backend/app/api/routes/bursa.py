...
from app.core.stocks import STOCK_UNIVERSE, MY_YF_TICKERS
...
async def _yf_quote_my(full_code: str) -> dict | None:
    ...
    try:
        ...
        logger.warning("yfinance KL fallback failed for %s: %s", full_code, e)
        return None


async def _fetch_quote(full_code: str) -> dict:
    """Fetch real-time quote for a MY stock, falling back to .KL yfinance."""
    ticker, market = _parse_code(full_code)
    meta = MY_STOCKS.get(full_code, {})

    try:
        q = await fetch_realtime_quote(ticker, market)
        # If moomoo returned a non-zero price, use it
        if q.get("last_price", 0) > 0:
            raw = q
        else:
            raw = (await _yf_quote_my(full_code)) or q
    except Exception:
...
async def bursa_market():
    ...
    quotes: list[dict] = await asyncio.gather(*[_fetch_quote(c) for c in MY_STOCKS])
    ...
    for q in active:
        ...
        if s not in sector_map:
            sector_map[s] = {"sector": s, "color": SECTOR_COLORS.get(s, "#4a7090"),
                              "count": 0, "total_change": 0.0, "stocks": []}
    ...
    await cache_set("bursa:market", result, ttl=30)
    return result


@router.get("/bursa/screener")
...
async def bursa_predict(code: str):
    ...
    try:
        ...
        macd_sig = float(pd.Series(close.ewm(span=12).mean() - close.ewm(span=26).mean()).ewm(span=9).mean().iloc[-1])
        ...
        await cache_set(f"bursa:predict:{full_code}", result, ttl=300)
        return result

    except Exception as e:
        logger.error("bursa_predict failed for %s: %s", full_code, e)
        return {
            "code": full_code, "ticker": ticker,
...
async def _sse_quote_generator(code: str):
    ticker, market = _parse_code(code)
    while True:
        try:
            q = await _fetch_quote(code)
            yield f"data: {json.dumps(q)}\n\n"
        except Exception as e:
            yield f"data: {json.dumps({'error': str(e), 'code': code})}\n\n"
